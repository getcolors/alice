...
launcher="$root/skills/package-alice-green/green"
...
grep -q 'io.github.getcolors.alice.workflow/workflow' "$launcher" || fail 'workflow dispatch is missing'
...
grep -qE '\(def \^:private alice-sha (nil|"[0-9a-f]{40}")\)' "$launcher" || fail 'invalid pin site'
ok 'has one managed immutable pin site'

mkdir "$tmp/bare"
cp "$launcher" "$tmp/bare/green"; chmod +x "$tmp/bare/green"
if grep -q '(def \^:private alice-sha nil)' "$launcher"; then
  out=$(cd "$tmp/bare" && ./green build 2>&1 || true)
  grep -q ALICE_LIB_ROOT <<<"$out" || fail 'an unpinned launcher did not explain ALICE_LIB_ROOT'
...
ok 'lifecycle, status, and tunnel commands are dispatchable'

[ -L "$root/green" ] && [ "$(readlink "$root/green")" = skills/package-alice-green/green ] || fail 'root green is not the payload symlink'
