(ns io.github.getcolors.alice.ssh-config
...
;; The marker alice wrote before it adopted the standard: the package name
;; ahead of the alias, saying "alice" twice. Retired, but not yet forgotten —
;; see `owned-markers`.
(defn superseded-begin-marker [alias] (str "# BEGIN alice " alias " ANSIBLE MANAGED BLOCK"))
(defn superseded-end-marker [alias] (str "# END alice " alias " ANSIBLE MANAGED BLOCK"))

(defn owned-markers
  "Every begin/end pair this package recognises as its own.

  A set rather than a pair because a marker change is a migration (standard
  §8). While one is in flight this holds the superseded marker too, so the
  ownership check below does not read alice's own old block as a hand-written
  stanza and refuse the very migration meant to clean it up.

  The superseded entries come out together with the removal task in
  `ansible-local/main.yml`, one pin cycle after this landed, or not at all."
  [alias]
