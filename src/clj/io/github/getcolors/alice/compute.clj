(ns io.github.getcolors.alice.compute
  "Alice requests one host on an existing network; colors-compute owns provider operations."
...
(defn requirements [opts]
  {:single_host true :private false :network {:mode "discovered"}
   :legacy_state_keys [(str (:profile opts) "/alice-infrastructure.tfstate")]
   :security {:egress "all" :private_filter false
              :ingress [{:id "ssh" :protocol "tcp" :from_port 22 :to_port 22
                         :sources (if (or (contains? opts :alice-ssh-sources) (contains? opts (keyword (str (:provider-compute opts) "-ssh-sources"))))
                                    (deployment/source-cidrs opts "ssh-sources" "alice-ssh-sources") ["0.0.0.0/0" "::/0"])}
