(ns pin
  "Stamp the bundled launcher with a clean pushed Alice commit."
  (:require [clojure.java.io :as io]
            [clojure.java.shell :as sh]
            [clojure.string :as str]))

(def ^:private site
  {:path "skills/package-alice-green/green"
   :rx #"\(def \^:private alice-sha (nil|\"[0-9a-f]{40}\")\)"})
  ...
  (let [file (io/file (:path site))]
    (cond
      ...
      (let [text (slurp file)
        ...
        (if-not current
          {:green/exit 2 :green/err (str "could not locate alice-sha in " (:path site))}
          (let [[head err] (repo-head "." "alice")]
            (cond
